export const DEFAULT_SESSION_TRASH_LIST_LIMIT = 500;
export const DEFAULT_SESSION_TRASH_PAGE_SIZE = 200;

export function resolveSessionTrashListLimit(context, constants = {}) {
    return Number(
        constants.sessionTrashListLimit
        ?? context?.sessionTrashListLimit
        ?? context?.SESSION_TRASH_LIST_LIMIT
    ) || DEFAULT_SESSION_TRASH_LIST_LIMIT;
}

export function resolveSessionTrashPageSize(context, constants = {}) {
    return Number(
        constants.sessionTrashPageSize
        ?? context?.sessionTrashPageSize
        ?? context?.SESSION_TRASH_PAGE_SIZE
...
export function createSessionTrashMethods({ api, constants = {} } = {}) {
    return {
        ...
        async loadSessionTrash(options = {}) {
            ...
            try {
                const res = await api('list-session-trash', {
                    limit: resolveSessionTrashListLimit(this, constants),
                ...
                if (this.isLatestSessionTrashListRequestToken(requestToken)) {
                    this.sessionTrashLastLoadFailed = true;
                    this.showMessage(e && e.message ? e.message : '加载回收站失败', 'error');
                }
            } finally {
        ...
        prependSessionTrashItem(item, options = {}) {
            ...
            const listLimit = resolveSessionTrashListLimit(this, constants);
            const pageSize = resolveSessionTrashPageSize(this, constants);
            ...
            const nextItems = [item, ...filtered].slice(0, listLimit);
        ...
        resetSessionTrashVisibleCount() {
            ...
            this.sessionTrashVisibleCount = Math.min(totalItems, pageSize) || pageSize;
        },
        ...
        loadMoreSessionTrashItems() {
            ...
            const safeVisibleCount = Number.isFinite(visibleCount) && visibleCount > 0
                ...
                : pageSize;
            this.sessionTrashVisibleCount = Math.min(totalItems, safeVisibleCount + pageSize);
        }
    };
}
