...
test('config template keeps expected config tabs in top and side navigation', () => {
    ...
    assert.match(taskOrchestrationStyles, /\.task-layout-grid-primary\s*\{[\s\S]*grid-template-columns:\s*minmax\(0, 1\.4fr\) minmax\(320px, 0\.8fr\);/);
    assert.match(taskOrchestrationStyles, /\.task-layout-grid-secondary\s*\{[\s\S]*grid-template-columns:\s*minmax\(0, 1fr\) minmax\(0, 1\.05fr\);/);
